...
class RainWaterTrapped():
    ...
    def solution_suffix_arr(self, A):
        """
        """

        n = len(A)

        # Calculate rightmax (suffix arr)
        ...
        for i in range(1, n):
            ...
            if (lmax < A[i]):
                lmax = A[i]
            print(f"I am at {i} and my left max is: {lmax}")
            print(f"I would trap {min(lmax, rmax[i]) - A[i]} water")
            ans += min(lmax, rmax[i]) - A[i]

        return ans
    
    def solution_optimal(self, A):
        """
        1. Iterate over the array, find the largest element of the array.
        2. Staning idx = 0, right max of all element upto largest is largest element itself
        3. Standing last index, largest element is left max of all upto it.
        4. Using these idea, calcuate and and update the answer.
        """
        ...
        water_trapped = 0
        # Find and update answer for 0 to lei index
        lmax = A[0]
        rmax = largest
        for i in range(0, lei):
            # Check and update lmax
            lmax = max(lmax, A[i])

            # Update the answer
            water_trapped += min(lmax, rmax) - A[i]

        # For Second half of the arr
        lmax = largest
        rmax = A[n-1]
        for i in range(n-2, lei, -1):
            # Check and update rmax
            rmax = max(rmax, A[i])
